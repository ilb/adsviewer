    ...
    const schema = {
      ...
      properties: {
        carmanufacturer: {
          ...
          type: 'string',
          options: conCat(
            delDuplicat(carsManufacturer.map(({ name }) => optionsValue(name))).sort(sortName)
          ),
          uniforms: {
            search: true
          }
            ...
            checkManuf(
              delDuplicat(
                carmodel.map(({ carmodel }) =>
                  carmodel.map(({ name }) => optionsValue(name)).sort(sortName)
                )[0]
              )
            )
          ),
        ...
        persons: {
          ...
          type: 'string',
          options: conCat(checkManuf(['1', '2', '3', '4', '5'].map((name) => optionsValue(name)))),
          uniforms: {
            search: true
          }
        },
        horse: {
          ...
          options: conCat(
            checkModel(
              delDuplicat(cardescription.map(({ enginepower }) => optionsValue(enginepower)))
            )
          ),
          uniforms: {
            search: true
        ...
        volume: {
          ...
          options: conCat(
            checkModel(
              delDuplicat(
                cardescription.map(({ enginecapacity }) => optionsValue(enginecapacity.toString()))
              )
            )
          )
        },
        region: {
          ...
          type: 'string',
          options: conCat(
            region.map(({ name, code }) => ({ value: code, label: name })).sort(sortName)
          ),
          uniforms: {
            search: true
          }
